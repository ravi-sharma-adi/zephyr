					       ...
					       uint16_t *rsp_len)
{
	...
	if (present != BT_BAS_BLS_BATTERY_NOT_PRESENT && present != BT_BAS_BLS_BATTERY_PRESENT) {
						  ...
						  uint16_t *rsp_len)
{
	...
	if (source < BT_BAS_BLS_WIRED_POWER_NOT_CONNECTED || source > BT_BAS_BLS_WIRED_POWER_UNKNOWN) {
		return BTP_STATUS_FAILED;
	}
						     ...
						     uint16_t *rsp_len)
{
	...
	if (source < BT_BAS_BLS_WIRELESS_POWER_NOT_CONNECTED || source > BT_BAS_BLS_WIRELESS_POWER_UNKNOWN) {
		return BTP_STATUS_FAILED;
	}
						    ...
						    uint16_t *rsp_len)
{
	...
	if (state < BT_BAS_BLS_CHARGE_STATE_UNKNOWN || state > BT_BAS_BLS_CHARGE_STATE_DISCHARGING_INACTIVE) {
		return BTP_STATUS_FAILED;
	}
						    ...
						    uint16_t *rsp_len)
{
	...
	if (level < BT_BAS_BLS_CHARGE_LEVEL_UNKNOWN || level > BT_BAS_BLS_CHARGE_LEVEL_CRITICAL) {
		return BTP_STATUS_FAILED;
	}
						   ...
						   uint16_t *rsp_len)
{
	...
	if (type < BT_BAS_BLS_CHARGE_TYPE_UNKNOWN || type > BT_BAS_BLS_CHARGE_TYPE_FLOAT) {
		return BTP_STATUS_FAILED;
	}
						...
						uint16_t *rsp_len)
{
	...
	if (value < BT_BAS_BLS_SERVICE_REQUIRED_FALSE || value > BT_BAS_BLS_SERVICE_REQUIRED_UNKNOWN) {
		return BTP_STATUS_FAILED;
	}
					     ...
					     uint16_t *rsp_len)
{
	...
	if (value != BT_BAS_BLS_BATTERY_FAULT_NO || value != BT_BAS_BLS_BATTERY_FAULT_YES) {
		return BTP_STATUS_FAILED;
	}
