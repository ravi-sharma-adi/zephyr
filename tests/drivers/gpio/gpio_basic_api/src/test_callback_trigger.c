...
static int test_callback(int mode)
{
	...
	gpio_pin_interrupt_configure(dev_in, PIN_IN, GPIO_INT_DISABLE);
	gpio_pin_interrupt_configure(dev_out, PIN_OUT, GPIO_INT_DISABLE);
	...
	int rc = gpio_pin_configure(dev_out, PIN_OUT, out_flags);
	...
	k_sleep(K_MSEC(1000));
	(void)gpio_pin_interrupt_configure(dev_in, PIN_IN, GPIO_INT_DISABLE);
