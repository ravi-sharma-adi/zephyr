...
ZTEST(video_common, test_video_ctrl)
{
	...
	zexpect_ok(video_set_ctrl(imager_dev, VIDEO_CID_CAMERA_EXPOSURE, (void *)30));
	zexpect_ok(video_get_ctrl(imager_dev, VIDEO_CID_CAMERA_EXPOSURE, &value));
	zexpect_equal(value, 30);

	/* Gain control, expected to be supported by all imagers */
	zexpect_ok(video_set_ctrl(imager_dev, VIDEO_CID_CAMERA_GAIN, (void *)30));
	zexpect_ok(video_get_ctrl(imager_dev, VIDEO_CID_CAMERA_GAIN, &value));
	zexpect_equal(value, 30);
}
