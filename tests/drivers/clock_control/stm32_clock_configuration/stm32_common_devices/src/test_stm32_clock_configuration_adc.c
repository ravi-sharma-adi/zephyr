...
#if defined(RCC_ADCCLKSOURCE_HSI)
#define ADC_SOURCE_HSI		RCC_ADCCLKSOURCE_HSI
#else
#define ADC_SOURCE_HSI		(-1)
#endif
...
ZTEST(stm32_common_devices_clocks, test_adc_clk_config)
{
	...
	if (IS_ENABLED(STM32_ADC_DOMAIN_CLOCK_SUPPORT) && DT_NUM_CLOCKS(DT_NODELABEL(adc1)) > 1) {
		...
		case STM32_SRC_SYSCLK:
			...
			break;
		case STM32_SRC_HSI:
			zassert_equal(dev_actual_clk_src, ADC_SOURCE_HSI,
					"Expected ADC1 src: HSI (0x%lx). Actual ADC1 src: 0x%x",
					ADC_SOURCE_HSI, dev_actual_clk_src);
			break;
