...
ZTEST(posix_thread_base, test_key_1toN_thread)
{
	void *retval;
...
ZTEST(posix_thread_base, test_key_Nto1_thread)
{
	pthread_t newthread;
...
ZTEST(posix_thread_base, test_key_resource_leak)
{
	pthread_key_t key;
...
ZTEST(posix_thread_base, test_correct_key_is_deleted)
{
	pthread_key_t key;
