...
static int soc_init(void)
{
	...
	struct scmi_clock_parent_config pclk_cfg = {0};
	...
	/* ENETREF clock init */
	pclk_cfg.clk_id = IMX95_CLK_ENETREF;
	pclk_cfg.parent_id = IMX95_CLK_SYSPLL1_PFD0;

	ret = scmi_clock_parent_set(proto, &pclk_cfg);
	if (ret) {
		return ret;
	}
