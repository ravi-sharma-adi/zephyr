...
static uint16_t calc_adv_interval(uint32_t sdu_interval_us)
{
	...
	if (BT_GAP_ADV_INTERVAL_TO_US(interval) != sdu_interval_us) {
		LOG_WRN("Advertising interval of 0x04%x is not a multiple of the advertising "
...
static int create_big(struct bt_le_ext_adv **adv, struct bt_iso_big **big)
{
	...
	const struct bt_le_adv_param *ext_adv_param =
		BT_LE_ADV_PARAM(BT_LE_ADV_OPT_EXT_ADV, adv_interval - BT_GAP_MS_TO_ADV_INT(10U),
				adv_interval - BT_GAP_MS_TO_ADV_INT(10U), NULL);
