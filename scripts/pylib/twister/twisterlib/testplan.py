    ...
    def handle_additional_scripts(self, platform_name: Platform, testsuide: TestInstance):
        ...
        if matched_scripting:
            ...
            for dut in self.env.hwm.duts:
                ...
                if dut.platform == platform_name and validate_boards(platform_name, matched_scripting.platforms):
                        ...
                        if script_obj and script_obj.get('path'):
                            ...
                            if Path(script_obj.get('path')).is_file():
                                ...
                                if script_obj.get('timeout') is not None:
                                    ...
                                    logger.info(f"{script_type} {script_obj.get('path')} will be executed on {platform_name} with timeout {script_obj.get('timeout')}")
                                else:
                                    logger.info(f"{script_type} {script_obj.get('path')} will be executed on {platform_name} with no timeout specified")
                            else:
                                raise TwisterRuntimeError(f"{script_type} script not found under path: {script_obj.get('path')}")
            return True
        return False
