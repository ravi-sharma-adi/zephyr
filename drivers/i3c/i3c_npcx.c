...
static int npcx_i3c_target_init(const struct device *dev)
{
	const struct npcx_i3c_config *config = dev->config;
	struct npcx_i3c_data *data = dev->data;
	struct i3c_config_controller *config_cntlr = &data->common.ctrl_config;
	struct i3c_reg *inst = config->base;
	uint8_t cntlr_dev_en;

	/* Enable secondary controller capability if supported */
	if (config_cntlr->is_secondary) {
		cntlr_dev_en = MCONFIG_CTRENA_CAPABLE; /* Controller mode capable */
	} else {
		cntlr_dev_en = MCONFIG_CTRENA_OFF; /* Controller mode off */
	}

	SET_FIELD(inst->MCONFIG, NPCX_I3C_MCONFIG_CTRENA, cntlr_dev_en);
...
static void npcx_i3c_target_isr(const struct device *dev)
{
	...
	if (IS_BIT_SET(inst->MINTMASKED, NPCX_I3C_MINTMASKED_NOWCNTLR)) {
		...
		config_cntlr->is_secondary = false;             /* Switch to primary controller */
...
static int npcx_i3c_init(const struct device *dev)
{
	...
	const struct device *const clk_dev = config->clock_dev;
	struct i3c_reg *inst = config->base;
	enum i3c_config_type config_type;
	int ret;
	...
	/* Initial the device as controller or target */
	config_type = (config_target->enable | config_cntlr->is_secondary) ? I3C_CONFIG_TARGET
									   : I3C_CONFIG_CONTROLLER;
	ret = (config_type == I3C_CONFIG_CONTROLLER) ? npcx_i3c_cntlr_init(dev)
						     : npcx_i3c_target_init(dev);
	if (ret != 0) {
		LOG_ERR("I3C device initial fail, ret=%d, type=%d", ret, config_type);
		return ret;
	}

	if (config_type == I3C_CONFIG_CONTROLLER) {
...
#define I3C_NPCX_DEVICE(id)                                                                        \
	...
	static struct npcx_i3c_data npcx_i3c_data_##id = {                                         \
		...
		.config_target.enable = DT_INST_PROP_OR(id, target_mode_en, false),                \
		...
		.config_target.bcr = DT_INST_PROP_OR(id, bcr, 0),                                  \
