					   ...
					   uint8_t *scll_od, uint8_t *sclh_i2c)
{
	if (i2c_bus_freq > 400000) {
		/* I2C bus is FM+ */
		*scll_od =
			DIV_ROUND_UP(STM32_I3C_SCLL_OD_MIN_FMP_NS * i3c_clock, 1000000000ull) - 1;
	} else if (i2c_bus_freq > 0) {
		/* I2C bus is FM */
		*scll_od = DIV_ROUND_UP(STM32_I3C_SCLL_OD_MIN_FM_NS * i3c_clock, 1000000000ull) - 1;
	} else {
		/* Assume no I2C devices on the bus */
		*scll_od =
			DIV_ROUND_UP(STM32_I3C_SCLL_OD_MIN_I3C_NS * i3c_clock, 1000000000ull) - 1;
	}
	...
	if (i2c_bus_freq > 400000 &&
	    *sclh_i2c <
		    DIV_ROUND_UP(STM32_I3C_SCLH_I2C_MIN_FMP_NS * i3c_clock, 1000000000ull) - 1) {
	...
	if (i2c_bus_freq > 0 &&
	    *sclh_i2c < DIV_ROUND_UP(STM32_I3C_SCLH_I2C_MIN_FM_NS * i3c_clock, 1000000000ull) - 1) {
		LOG_ERR("Cannot find a combination of SCLL_OD and SCLH_I2C at current I3C clock "
			"frequency for FM I2C bus");
		return -EINVAL;
	}
