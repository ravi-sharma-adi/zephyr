...
BUILD_ASSERT(ASYNC_UART_STATUS_TIMEOUT != DMA_STATUS_COMPLETE,
	     "ASYNC_UART_STATUS_TIMEOUT must be different from DMA_STATUS_COMPLETE");
BUILD_ASSERT(ASYNC_UART_STATUS_TIMEOUT != DMA_STATUS_BLOCK,
	     "ASYNC_UART_STATUS_TIMEOUT must be different from DMA_STATUS_BLOCK");
#endif
...
static int uart_stm32_async_init(const struct device *dev)
{
	...
	if (data->dma_rx.dma_cfg.cyclic == 1) {
	...
	if (data->dma_tx.dma_cfg.cyclic == 1) {
		/* TX enable circular buffer */
