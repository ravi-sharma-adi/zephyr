...
static void lsm6dsv16x_config_fifo(const struct device *dev, uint8_t enable)
{
...
static void lsm6dsv16x_read_fifo_cb(struct rtio *r, const struct rtio_sqe *sqe, void *arg)
{
	...
	if (data_opt == SENSOR_STREAM_DATA_NOP || data_opt == SENSOR_STREAM_DATA_DROP) {
		...
		rx_data->header.timestamp = k_ticks_to_ns_floor64(k_uptime_ticks());
	...
	rtio_sqe_prep_callback(complete_op,
			       lsm6dsv16x_complete_op_cb, (void *)dev, lsm6dsv16x->streaming_sqe);
