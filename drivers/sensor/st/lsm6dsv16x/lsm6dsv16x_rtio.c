...
static void lsm6dsv16x_submit_sample(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	for (int i = 0; i < num_channels; i++) {
		...
		case SENSOR_CHAN_ACCEL_XYZ:
			...
			if (lsm6dsv16x_acceleration_raw_get(ctx, edata->acc) < 0) {
		...
		case SENSOR_CHAN_GYRO_XYZ:
			...
			if (lsm6dsv16x_angular_rate_raw_get(ctx, edata->gyro) < 0) {
		...
		case SENSOR_CHAN_DIE_TEMP:
			...
			if (lsm6dsv16x_temperature_raw_get(ctx, &edata->temp) < 0) {
