...
static int gpio_rts5912_configuration(const struct device *port, gpio_pin_t pin, gpio_flags_t flags)
{
	...
	switch (flags & GPIO_VOLTAGE_MASK) {
	case GPIO_VOLTAGE_1P8:
		cfg_val |= GPIO_GCR_INVOLMD_Msk;
		break;
	case GPIO_VOLTAGE_DEFAULT:
	case GPIO_VOLTAGE_3P3:
		cfg_val &= ~GPIO_GCR_INVOLMD_Msk;
		break;
	case GPIO_VOLTAGE_5P0:
		return -ENOTSUP;
	default:
		break;
	}
	...
	if (*gcr & GPIO_GCR_OUTCTRL_Msk) {
		...
		if (*gcr & GPIO_GCR_DIR_Msk) {
			...
			if (*gcr & GPIO_GCR_INVOLMD_Msk) {
				cfg_flag |= GPIO_VOLTAGE_1P8;
			} else {
				cfg_flag |= GPIO_VOLTAGE_3P3;
			}
		}
	}
...
static const struct gpio_driver_api gpio_rts5912_driver_api = {
