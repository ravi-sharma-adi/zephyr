...
static int sent_nxp_s32_start(const struct device *dev, uint8_t channel)
{
	const struct sent_nxp_s32_config *config = dev->config;
...
static int sent_nxp_s32_stop(const struct device *dev, uint8_t channel)
{
	const struct sent_nxp_s32_config *config = dev->config;
	struct sent_nxp_s32_data *data = dev->data;
	struct sent_nxp_s32_channel_data *channel_data = &data->channel_data[channel];
...
#define SENT_NXP_S32_CALLBACK(n)                                                                   \
	...
	{                                                                                          \
		...
		ARG_UNUSED(event);                                                                 \
                                                                                                   \
		if (rx_callback->callback) {                                                       \
			rx_callback->callback(dev, channel, NULL, SENT_STATE_FMSG_ERR,             \
	...
	{                                                                                          \
		...
		ARG_UNUSED(event);                                                                 \
                                                                                                   \
		if (rx_callback->callback) {                                                       \
			rx_callback->callback(dev, channel, NULL, SENT_STATE_SMSG_ERR,             \
					      rx_callback->user_data);                             \
		}                                                                                  \
	}
