...
extern fsp_err_t r_sdhi_command_send(sdhi_instance_ctrl_t *p_ctrl, uint32_t command,
				     uint32_t argument);
...
void r_sdhi_callback(sdmmc_callback_args_t *p_args)
{
	if (p_args->event == SDMMC_EVENT_TRANSFER_COMPLETE) {
		...
		sdmmc_event.transfer_error = false;
	} else if (p_args->event == SDMMC_EVENT_TRANSFER_ERROR) {
		sdmmc_event.transfer_error = true;
...
static int sdhc_ra_send_cmd(struct sdhc_ra_priv *priv, struct sdmmc_ra_command *ra_cmd, int retries)
{
	...
	while (retries > 0) {
		fsp_err = r_sdhi_command_send(&priv->sdmmc_ctrl, ra_cmd->opcode, ra_cmd->arg);
		if (fsp_err != 0) {
			retries--; /* error, retry */
		} else {
			break;
			   ...
			   struct sdhc_data *data)
{
	...
	if (data) {
		...
		ra_cmd.sector_start = data->block_addr;
	...
	case SD_SWITCH:
		...
		if (priv->app_cmd && cmd->opcode == SD_APP_SET_BUS_WIDTH) {
			...
			fsp_err = r_sdhi_read_and_block(&priv->sdmmc_ctrl, ra_cmd.opcode,
							ra_cmd.arg, data->block_size);
			ret = err_fsp2zep(fsp_err);
			if (ret < 0) {
				goto end;
			}
			memcpy(data->data, priv->sdmmc_ctrl.aligned_buff, 8);
			sdmmc_event.transfer_completed = false;
			sdmmc_event.transfer_error = false;
			break;
		}
	...
	case SD_READ_MULTIPLE_BLOCK:
		...
		fsp_err = r_sdhi_transfer_read(&priv->sdmmc_ctrl, ra_cmd.sector_count,
					       (uint32_t)priv->fsp_config.block_size, data->data);
		ret = err_fsp2zep(fsp_err);
		if (ret < 0) {
			goto end;
		}
		...
		r_sdhi_read_write_common(&priv->sdmmc_ctrl, ra_cmd.sector_count,
					 (uint32_t)priv->fsp_config.block_size, ra_cmd.opcode,
					 ra_cmd.arg);
...
static int sdhc_ra_init(const struct device *dev)
{
	...
	priv->sdmmc_ctrl.device.sector_size_bytes = priv->sdmmc_ctrl.p_cfg->block_size;
...
static const struct sdhc_driver_api sdhc_api = {.reset = sdhc_ra_reset,
