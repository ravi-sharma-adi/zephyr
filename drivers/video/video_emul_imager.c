...
static const uint16_t pattern_8bars_yuv[8][3] = {
	{0x00, 0x7F, 0x7F}, {0x00, 0xFF, 0x00}, {0x00, 0x00, 0xFF}, {0x00, 0xFF, 0xFF},
	{0x7F, 0x00, 0x00}, {0xFF, 0xFF, 0x00}, {0xFF, 0x00, 0xFF}, {0xFF, 0x7F, 0x7F}};
...
static void emul_imager_fill_framebuffer(const struct device *const dev, struct video_format *fmt)
{
	...
	case VIDEO_PIX_FMT_RGB565:
		for (size_t i = 0; i < fmt->width; i++) {
			r = pattern_8bars_rgb[i * 8 / fmt->width][0] >> 3;
			g = pattern_8bars_rgb[i * 8 / fmt->width][1] >> 2;
			b = pattern_8bars_rgb[i * 8 / fmt->width][2] >> 3;
			fb16[i] = sys_cpu_to_le16(r << 11 | g << 5 | b << 0);
		}
		break;
	default:
...
int emul_imager_init(const struct device *dev)
{
	...
	if (/* !i2c_is_ready_dt(&conf->i2c) */ false) {
...
#define EMUL_IMAGER_DEFINE(inst)                                                                   \
	static struct emul_imager_data emul_imager_data_##inst;                                    \
                                                                                                   \
	static const struct emul_imager_config emul_imager_conf_##inst = {                         \
		.i2c = /* I2C_DT_SPEC_INST_GET(inst) */ {0},                                       \
	};                                                                                         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, &emul_imager_init, NULL, &emul_imager_data_##inst,             \
			      &emul_imager_conf_##inst, POST_KERNEL, CONFIG_VIDEO_INIT_PRIORITY,   \
