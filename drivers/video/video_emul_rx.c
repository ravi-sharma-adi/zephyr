...
{
	const struct emul_rx_config *conf = dev->config;
			       ...
			       struct video_frmival *frmival)
{
	...
	return video_set_frmival(conf->source_dev, VIDEO_EP_OUT, frmival);
}
			       ...
			       struct video_frmival *frmival)
{
	...
	return video_get_frmival(conf->source_dev, VIDEO_EP_OUT, frmival);
}
				...
				struct video_frmival_enum *fie)
{
	...
	return video_enum_frmival(conf->source_dev, VIDEO_EP_OUT, fie);
}
			   ...
			   struct video_format *fmt)
{
	...
	ret = video_set_format(conf->source_dev, VIDEO_EP_OUT, fmt);
	if (ret < 0) {
		LOG_DBG("Failed to set %s format to %x %ux%u", conf->source_dev->name,
			    ...
			    struct video_caps *caps)
{
	...
	return video_get_caps(conf->source_dev, VIDEO_EP_OUT, caps);
}
...
static int emul_rx_stream_stop(const struct device *dev)
{
	...
	return video_stream_stop(conf->source_dev);
...
#define EMUL_RX_DEFINE(n)                                                                          \
	static const struct emul_rx_config emul_rx_conf_##n = {                                    \
		.source_dev = DEVICE_DT_GET_REMOTE_DEVICE(DT_INST_ENDPOINT_BY_ID(n, 0, 1)),        \
	};                                                                                         \
                                                                                                   \
	static struct emul_rx_data emul_rx_data_##n = {                                            \
		.dev = DEVICE_DT_INST_GET(n),                                                      \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, &emul_rx_init, NULL, &emul_rx_data_##n, &emul_rx_conf_##n,        \
