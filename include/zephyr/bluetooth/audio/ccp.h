...
#endif
/**
 * @defgroup bt_ccp_server CCP Server APIs
 * @ingroup bt_ccp
 * @{
 */
 ...
 * bt_ccp_server_unregister_bearer().
 *
 ...
 * bt_ccp_server_register_bearer() before it can be unregistered.
 *
...
struct bt_ccp_client_bearers {
	...
	struct bt_ccp_client_bearer *tbs_bearers[CONFIG_BT_CCP_CLIENT_BEARER_COUNT];
 ...
 */
struct bt_ccp_client_cb {
	/**
	 * @brief Callback function for bt_ccp_client_discover().
	 *
...
int bt_ccp_client_discover(struct bt_conn *conn, struct bt_ccp_client **out_client);
 ...
 */
int bt_ccp_client_register_cb(struct bt_ccp_client_cb *cb);
