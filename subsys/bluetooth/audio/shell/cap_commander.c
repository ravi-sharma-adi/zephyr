						       ...
						       char *argv[])
{
	 ...
	 */
	if (cap_args != conn_cnt + 1) {
		shell_error(sh, "Cannot use %zu arguments for %zu connections", argc, conn_cnt);
		return -ENOEXEC;
	}

	/* the last argument is the broadcast code */
	if (strlen(argv[argc]) > BT_ISO_BROADCAST_CODE_SIZE) {
	...
	memcpy(param.broadcast_code, argv[argc], strlen(argv[argc]));
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(distribute_broadcastcode, NULL,
		      "Distribute broadcast code <src_id [...]> <broadcast code",
