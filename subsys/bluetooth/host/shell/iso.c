...
static void iso_connected(struct bt_iso_chan *chan)
{
	...
	if (err != 0) {
		shell_error(ctx_shell, "Failed to get ISO info: %d", err);
		return;
	}
	...
	if (iso_info.can_recv) {
		...
		if (err != 0) {
			shell_error(ctx_shell, "Failed to setup ISO RX data path: %d", err);
		}
	}
	...
	if (iso_info.can_send) {
		...
		if (err != 0) {
			shell_error(ctx_shell, "Failed to setup ISO TX data path: %d", err);
		}
	}
}
...
static void iso_disconnected(struct bt_iso_chan *chan, uint8_t reason)
{
	...
	if (err != 0) {
		...
		if (iso_info.can_recv) {
			...
			if (err != 0) {
				shell_error(ctx_shell, "Failed to remove ISO RX data path: %d",
					    err);
			}
		}
		...
		if (iso_info.can_send) {
			...
			if (err != 0) {
				shell_error(ctx_shell, "Failed to remove ISO TX data path: %d",
					    err);
			}
		}
	}
