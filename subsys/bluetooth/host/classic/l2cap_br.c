...
static void l2cap_br_monitor_timeout(struct k_work *work)
{
	...
	if (br_chan->tx.mode == BT_L2CAP_BR_LINK_MODE_ERET) {
		if (!atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P_CHANGED) &&
			...
			br_chan->retry_count++;
			if (br_chan->tx.transmit &&
			    (br_chan->retry_count > br_chan->tx.transmit)) {
				bt_l2cap_chan_disconnect(&br_chan->chan);
				return;
			}
...
static int bt_l2cap_br_pack_s_frame_header(struct bt_l2cap_br_chan *br_chan,
		...
		if (!br_chan->_pdu_buf) {
			...
			if (alloc) {
				...
				if (send_buf == NULL) {
					 ...
					 */
					LOG_DBG("no buf for sending S-frame on %p", br_chan);
					return NULL;
				}
			}
			...
			if (send_buf != NULL) {
				...
				if (err < 0) {
					...
					LOG_DBG("Cannot sending S-frame on %p", br_chan);
					return NULL;
				}
...
send_i_frame:
		...
		if (first) {
			...
			LOG_WRN("Sending I-frame %u: buf %p chan %p len %zu", tx_win->tx_seq,
					pdu, br_chan, pdu_len);
		} else {
			...
			if (br_chan->tx.mode == BT_L2CAP_BR_LINK_MODE_ERET) {
				if (br_chan->tx.transmit &&
				    (tx_win->transmit_counter > br_chan->tx.transmit)) {
					disconnect = true;
				}
			} else {
				   ...
				   struct net_buf *buf, size_t len)
{
	...
	if (opt_ret_fc->mode != BT_L2CAP_BR_LINK_MODE_BASIC) {
		...
		br_chan->rx.monitor_timeout = CONFIG_BT_L2CAP_BR_MONITOR_TIMEOUT;
		br_chan->rx.transmit = opt_ret_fc->max_transmit;
		br_chan->rx.window = opt_ret_fc->tx_windows_size;
		br_chan->rx.mps = sys_le16_to_cpu(opt_ret_fc->mps);
	}
					 ...
					 struct net_buf *buf, size_t len)
{
	...
	if (opt_ret_fc->mode != BT_L2CAP_BR_LINK_MODE_BASIC) {
		...
		br_chan->tx.window = opt_ret_fc->tx_windows_size;
				   ...
				   struct net_buf *buf, size_t len)
{
	...
	case BT_L2CAP_BR_LINK_MODE_ERET:
		...
		br_chan->tx.extended_control = true;
		br_chan->tx.window = sys_le16_to_cpu(opt_ext_win_size->max_windows_size);
		break;
	case BT_L2CAP_BR_LINK_MODE_STREAM:
		if (win_size) {
			result = BT_L2CAP_CONF_UNACCEPT;
