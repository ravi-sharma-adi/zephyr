...
static void l2cap_connected(struct bt_l2cap_chan *chan)
{
	...
	case BT_L2CAP_BR_LINK_MODE_BASIC:
		shell_print(ctx_shell, "It is basic mode");
		if (br_chan->hold_credit) {
			br_chan->hold_credit = false;
			shell_warn(ctx_shell, "hold_credit is unsupported in basic mode");
		}
		break;
	case BT_L2CAP_BR_LINK_MODE_RET:
		shell_print(ctx_shell, "It is retransmission mode");
		break;
	case BT_L2CAP_BR_LINK_MODE_FC:
		shell_print(ctx_shell, "It is flow control mode");
		break;
	case BT_L2CAP_BR_LINK_MODE_ERET:
		shell_print(ctx_shell, "It is enhance retransmission mode");
		break;
	case BT_L2CAP_BR_LINK_MODE_STREAM:
		shell_print(ctx_shell, "It is streaming mode");
		break;
	default:
		shell_error(ctx_shell, "It is unknown mode");
		break;
	}
#endif /* L2CAP_BR_RET_FC_ENABLE */
	(void)br_chan;
			...
			struct bt_l2cap_chan **chan)
{
	...
	*chan = &l2cap_chan.chan.chan;
	...
	if (br_server->options & BT_L2CAP_BR_SERVER_OPT_STREAM) {
		...
		l2cap_chan.chan.rx.window = CONFIG_BT_L2CAP_MAX_WINDOW_SIZE;
		l2cap_chan.chan.rx.transmit = 0;
	} else if (br_server->options & BT_L2CAP_BR_SERVER_OPT_ERET) {
		l2cap_chan.chan.rx.mode = BT_L2CAP_BR_LINK_MODE_ERET;
		l2cap_chan.chan.rx.window = CONFIG_BT_L2CAP_MAX_WINDOW_SIZE;
		l2cap_chan.chan.rx.transmit = 3;
