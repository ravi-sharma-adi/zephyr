...
static int bt_hfp_ag_bind_handler(struct bt_hfp_ag *ag, struct net_buf *buf)
{
	...
	if (is_char(buf, '?')) {
		...
		hfp_ag_unlock(ag);
		len = (sizeof(supported_indicators) * 8) > HFP_HF_IND_MAX ? HFP_HF_IND_MAX
			   : (sizeof(supported_indicators) * 8);
		for (int i = 1; i < len; i++) {
			bool enabled;
...
static int hfp_ag_accept(struct bt_conn *conn, struct bt_rfcomm_dlc **dlc)
{
	struct bt_hfp_ag *ag;
